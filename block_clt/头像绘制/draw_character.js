...
function draw_character(move,loc,chara_image_1){
	...
	localStorage.character = "挑战者;1;#0099FF/;3/3;3/2";
	...
	if(move){
		drawBgColor(loc,map_array,map_info,cxt,bgColor,bgColor,1);
		cxt.drawImage(chara_image_1,loc.x-40,loc.y-30,80,60);
	}else{
		var loc ={x:map_array[loc_num].x,y:map_array[loc_num].y};
		drawBgColor(loc,map_array,map_info,cxt,bgColor,bgColor,0);
		console.log("绘制绵羊");
		cxt.drawImage(chara_image_1,map_array[loc_num].x-40,map_array[loc_num].y-30,80,60);
		console.log("绘制结束了");
	}
